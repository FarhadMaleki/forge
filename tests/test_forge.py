...
from forge.utils import image_equal
from forge.utils import referenced_3D_resample
from forge.utils import refrence_free_3D_resample
...
class TestTransform(unittest.TestCase):
    ...
    def test_Pad(self):
        ...
        padding = (1, 1, 1)
        tsfm = forge.Pad(padding, method='constant', constant=None,
        ...
        padding = (1, 1, 1)
        constant = 1024
        tsfm = forge.Pad(padding, method='constant', constant=constant,
                         background_label=3, pad_lower_bound=True,
                         pad_upper_bound=True, p=1.0)
        img, msk = tsfm(image, mask=mask)
        self.assertEqual(img.GetSize(),
        ...
        # Mirror padding
        padding = (1, 2, 3)
        tsfm = forge.Pad(padding, method='mirror',
                         background_label=3, pad_lower_bound=True,
                         pad_upper_bound=True, p=1.0)
        img, msk = tsfm(image, mask=mask)
    ...
    def test_Af(self):
        image, mask = TestTransform.load_cube('hole')
        tsfm = forge.Affine(angles=(0, 0, 30),
                            translation=(0, 0, 0),
                            scales=(1, 1, 1),
    ...
    def test_only_rotation_Affine(self):
        image, mask = TestTransform.load_cube('small')
        tsfm = forge.Affine(angles=(180, 0, 0),
                            translation=(0, 0, 0),
                            scales=(1, 1, 1),
                            interpolator=sitk.sitkLinear,
                            image_background=-1024,
    ...
    def test_only_rotation_and_Scaling_Affine(self):
        image, mask = TestTransform.load_cube('small')
        scale = (2, 2, 2)
        tsfm = forge.Affine(angles=(1, 1, 1),
                            translation=(0, 0, 0),
                            scales=scale,
                            interpolator=sitk.sitkLinear,
                            image_background=-1024,
    ...
    def test_RandomCrop_not_possible(self):
        ...
        size[0] += 1
        tsfm = forge.RandomCrop(tuple(size), p=1)
        try:
    ...
    def test_CenterCrop_not_possible(self):
        ...
        size[0] += 1
        try:
            tsfm = forge.CenterCrop(tuple(size), p=1)
            tsfm(image, mask)
            assert False
        except ValueError as e:
    ...
    def test_SegmentSafeCrop(self):
        image, mask = TestTransform.load_cube('small')
        tsfm = forge.RandomSegmentSafeCrop(crop_size=(7, 7, 7),
                                           include=[1], p=1.0)
        img, msk = tsfm(image, mask=mask)
        self.assertEqual(sitk.GetArrayFromImage(image).sum(),
    ...
    def test_SegmentSafeCrop_not_possible(self):
        ...
        tsfm = forge.RandomSegmentSafeCrop(crop_size=crop_size,
                                           include=[1], p=1.0)
        try:
            tsfm(image, mask=mask)
        except ValueError as e:
    ...
    def test_RandomRotation(self):
        image, mask = TestTransform.load_cube('hole')
        tsfm = forge.RandomRotation((0, 0, 45),
                                    interpolator=sitk.sitkBSpline,
                                    image_background=-1024,
                                    mask_background=0,
                                    reference=None)
    ...
    def test_BinomialBlur(self):
        ...
        tsfm = forge.BinomialBlur(repetition=3, p=1.0)
        img, msk = tsfm(image, mask=mask)
    ...
    def test_Writer(self):
        image, mask = TestTransform.load_cube('small')
        writer = forge.SequentialWriter(dir_path='tests/data',
